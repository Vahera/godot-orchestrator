                                                                        ...
                                                                        bool p_function_entry)
{
    ...
    if (p_function_entry)
    {
        for (const PropertyInfo& property : p_function->get_method_info().arguments)
        {
            ...
            Ref<OScriptNodePin> pin = create_output_pin(PT_Data, property);
    ...
    else
    {
        ...
        {
            Ref<OScriptNodePin> pin = create_input_pin(PT_Data, "return_value", p_function->get_return_type());
            pins_good = pin.is_valid() & pins_good;

            // Create hidden output pin to transfer value to caller
            Ref<OScriptNodePin> out = create_output_pin(PT_Data, "return_out", p_function->get_return_type());
