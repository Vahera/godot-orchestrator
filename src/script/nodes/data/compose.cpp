...
void OScriptNodeCompose::allocate_default_pins()
{
    ...
    for (int i = 0; i < components.size(); i++)
    {
        ...
        create_input_pin(PT_Data, components[i], bit.get_type());
    }

    // This is the pin that will be constructed from its types
    create_output_pin(PT_Data, "value", _type);
}
...
void OScriptNodeComposeFrom::allocate_default_pins()
{
    for (int i = 0; i < _constructor_args.size(); i++)
    {
        ...
        const StringName pin_name = property.name.is_empty() ? StringName("arg" + itos(i)) : property.name;
        create_input_pin(PT_Data, pin_name, property);
    }
