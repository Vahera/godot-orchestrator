...
void OrchestratorScriptView::_notification(int p_what)
{
    if (p_what == NOTIFICATION_READY)
    {
        ...
        _graphs->connect("scroll_to_item", callable_mp(this, &OrchestratorScriptView::_on_scroll_to_item).bind(sc));
        ...
        _functions->connect("scroll_to_item", callable_mp(this, &OrchestratorScriptView::_on_scroll_to_item).bind(sc));
        vbox->add_child(_functions);

        _macros = memnew(OrchestratorScriptViewMacrosSection(_script));
        _macros->connect("scroll_to_item", callable_mp(this, &OrchestratorScriptView::_on_scroll_to_item).bind(sc));
        vbox->add_child(_macros);

        _variables = memnew(OrchestratorScriptViewVariablesSection(_script));
        _variables->connect("scroll_to_item", callable_mp(this, &OrchestratorScriptView::_on_scroll_to_item).bind(sc));
        vbox->add_child(_variables);

        _signals = memnew(OrchestratorScriptViewSignalsSection(_script));
        _signals->connect("scroll_to_item", callable_mp(this, &OrchestratorScriptView::_on_scroll_to_item).bind(sc));
